...
heure_hiver = -1
heure_ete = -2
decal_heure_ete_hiver_gmt = heure_hiver
...
reader = PdfReader('planning_admr.pdf')
...
for ligne in result_par_ligne:
	...
	if is_day_name(ligne):
		...
		str_debut_heure = str(int(ligne[len_mois+8:len_mois+10])+decal_heure_ete_hiver_gmt)
		if verbose:
		...
		str_fin_heure = str(int(ligne[len_mois+16:len_mois+18])+decal_heure_ete_hiver_gmt)
		if verbose:
		...
		# écriture de l'évènement au format ics/google dans le fichier admr.ics
